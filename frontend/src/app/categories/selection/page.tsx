...
import { fetchData, subscribeToUpdates } from '@/utils/api';
...
const CategorySelectionPage: React.FC = () => {
  ...
  // Define fetchCategories function outside useEffect so it can be used by handleRetry
  const fetchCategories = async (skipCache: boolean = false): Promise<void> => {
      setLoading(true);
      try {
        // Fetch categories from our API endpoint
        const endpoint = CATEGORY_ENDPOINTS.ALL;
        const response = await fetchData<APIResponse>(endpoint, {}, !skipCache);
        
        // Debug the response structure
        console.log('API Response:', response);
        
        // Validate response data to prevent errors
        if (!response) {
          console.warn('Empty API response, using fallback data');
        ...
        if (!response.categories || !Array.isArray(response.categories)) {
          console.warn('Invalid categories data format, using fallback data');
          setCategories(fallbackCategories);
          return;
        }
        
        // Process API data with additional safety checks
        const enhancedCategories: Category[] = response.categories.map((cat: APICategory) => ({
          ...
          newItems: Number(cat.newItems) || 0
        }));
        ...
        setError(`Unable to load categories: ${errorMessage}. Showing fallback data.`);
        setCategories(fallbackCategories);
        
        // Show error toast
        toast({
          title: 'Error loading categories',
          description: `${errorMessage}. Using fallback data.`,
          status: 'warning', // Changed from error to warning since we have fallback
          duration: 5000,
          isClosable: true,
        });
      } finally {
  ...
  // Handle retry - fetch fresh data instead of full page reload
  const handleRetry = () => {
    fetchCategories(true); // Skip cache to get fresh data
  };
  
  // Create a masonry-style grid layout
  const createMasonryGrid = () => {
    // Ensure categories is an array before attempting to use it
